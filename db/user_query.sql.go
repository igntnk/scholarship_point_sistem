...
const addUserToUserGroup = `-- name: AddUserToUserGroup :exec
...
func (q *Queries) AddUserToUserGroup(ctx context.Context, userUuid pgtype.UUID) error {
	_, err := q.db.Exec(ctx, addUserToUserGroup, userUuid)
	return err
}
...
select u.uuid, name, second_name, patronymic, gradebook_number, birth_date, email, phone_number, status_uuid, password, salt, s.uuid, internal_value, display_value, type
from sys_user u
...
func (q *Queries) MakeUserUnverified(ctx context.Context, uuid pgtype.UUID) error {
	_, err := q.db.Exec(ctx, makeUserUnverified, uuid)
	return err
}
...
func (q *Queries) MakeUserVerified(ctx context.Context, uuid pgtype.UUID) error {
	_, err := q.db.Exec(ctx, makeUserVerified, uuid)
	return err
}
